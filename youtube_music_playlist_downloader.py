...
def check_ffmpeg():
    # Check if local or WinGet ffmpeg exists and add to PATH if so
    ...
    # Check User Profile directory for winget installed ffmpeg packages
    user_profile = os.environ.get("USERPROFILE") or os.environ.get("HOME")
    if user_profile:
        winget_pkg_dir = os.path.join(user_profile, "AppData", "Local", "Microsoft", "WinGet", "Packages")
        if os.path.exists(winget_pkg_dir):
            try:
                for pkg_name in os.listdir(winget_pkg_dir):
                    if "ffmpeg" in pkg_name.lower():
                        pkg_path = os.path.join(winget_pkg_dir, pkg_name)
                        for root, dirs, files in os.walk(pkg_path):
                            if "ffmpeg.exe" in files or "ffmpeg" in files:
                                local_paths.append(root)
                                break
            except Exception:
                pass
...
def get_playlist_info(config: dict):
    # Rewrite watch URLs with a playlist ID to ensure extract_flat extracts the playlist entries instead of the single video
    url = config["url"]
